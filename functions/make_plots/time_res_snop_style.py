...
for Ecut_i in E_cut_list:
    ...
    for i_dx, Rcut_i in enumerate(R_cut_list):
        ...
        # ==========================================
        # Shaded vertical span with dashed boundary lines
        ax.axvspan(t_res_cher_min, t_res_cher_max, color='gray', alpha=0.2, 
        ...
        ax.hist(bin_edges[:-1], bins=bin_edges, weights=counts, density=True, 
                label=r'$^8$B-$\nu_e$ MC', **mc_histogram_style)

        # Labels & limits
        ax.set_xlabel(r'$t_{\rm res}$ (ns)', size=26)
