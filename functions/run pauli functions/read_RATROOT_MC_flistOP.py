...
def extract_data(read_dir, file_txt_dir, save_dir):
	...
	hit_pmtid = ROOT.std.vector('int')()
	tree.Branch('hit_pmtid', hit_pmtid)

	hit_residual = ROOT.std.vector('double')()
	tree.Branch('hit_residual', hit_residual)
	...
	for input_file_i in flist:
		...
		for ievent in range(0, reader.GetEntryCount()):
			...
			for iev in range(0, rDS.GetEVCount()):
				...
				# Clear std.vac before saving the per hit observables
				hit_pmtid.clear()
				hit_residual.clear()
				...
				for ivtx in range(0, fResult.GetVertexCount()):
					...
					for iPMT in range(0, calibratedPMTs.GetAllCount()):
						...
						hit_pmtid.push_back(pmtid)
						hit_residual.push_back(residual)
...
if __name__ == "__main__":
	...
	save_dir = '/lstore/sno/joankl/solar_analysis/mc_data/main_simulations/bisMSB/B8_solar_Nue/proof/xOP.root'
