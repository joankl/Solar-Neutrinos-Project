...
def Analysis_simulation(file, evID = False):
	...
	mc_momentum_ID = []									# simulated event direction
	...
	for job_j in job:
		...
		mc_momentum_job = locals()['mc_momentum_'+str(job_j)][0]
		...
		mc_momentum_job1 = []									# simulated event direction of job
		...
		for i_id in np.unique(evID_job):
			...
			for i in where(condition_ev)[0]:
				...
				mc_momentum_ev.append(mc_momentum_job[i])
			...
			mc_momentum_job1.append(mc_momentum_ev[0])
		...
		mc_momentum_ID.append(mc_momentum_job1)
